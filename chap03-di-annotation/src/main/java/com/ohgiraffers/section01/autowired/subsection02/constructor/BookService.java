...
public class BookService {
    ...
    public List<BookDTO> selectAllBooks(){ return bookDAO.selectBookList(); }
    public BookDTO searchBookBySequence(int sequence){ return bookDAO.selectOneBook(sequence); }

    /*
    * spring 4.3버전 이후로는 생성자가 한 개 뿐이라면 @Autowired 어노테이션을 생략해도 자동으로 생성자 주입이 동작한다.
    ...
    *   - 순환 참조에 대해 필드 주입/세터 주입은 메소드를 실행 시점에 오류가 발생한다.
